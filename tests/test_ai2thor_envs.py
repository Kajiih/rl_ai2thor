...
from rl_thor.envs._config import EnvConfig, InvalidActionGroupError  # noqa: PLC2701
from rl_thor.envs.actions import ActionGroup, EnvActionName
from rl_thor.envs.ai2thor_envs import (
    ITHOREnv,
    NoTaskBlueprintError,
)
from rl_thor.envs.sim_objects import SimObjectType
from rl_thor.envs.tasks.tasks import PlaceIn, PlaceNSameIn, UnknownTaskTypeError
...
@pytest.fixture()
def ithor_env_2(config_override=None):
    env = ITHOREnv(config_override=config_override)
    yield env
    env.close()
...
def test__compute_action_availabilities():
    expected_availabilities = {
        EnvActionName.MOVE_FORWARD: True,
        EnvActionName.MOVE_BACKWARD: False,
        ...
        EnvActionName.PUT_DOWN_OBJECT: True,
        EnvActionName.TOGGLE_OBJECT_ON: True,
        EnvActionName.TOGGLE_OBJECT_OFF: True,
        EnvActionName.CROUCH_DOWN: False,
        EnvActionName.STAND_UP: False,
        EnvActionName.MOVE_HELD_OBJECT_FORWARD_BACKWARD: False,
        EnvActionName.MOVE_HELD_OBJECT_LEFT_RIGHT: False,
        ...
        EnvActionName.DROP_HELD_OBJECT: False,
        EnvActionName.THROW_HELD_OBJECT: False,
        EnvActionName.PUSH_OBJECT: False,
        EnvActionName.PULL_OBJECT: False,
        EnvActionName.FILL_OBJECT_WITH_LIQUID: False,
        EnvActionName.EMPTY_LIQUID_OBJECT: False,
...
@pytest.mark.parametrize("config_override", [partial_config_dict])
def test__action_space(ithor_env):
...
@pytest.mark.parametrize(
    "config_override",
    [{"controller_parameters": {"frame_height": 84, "frame_width": 44}}],
)
def test__create_observation_space(ithor_env):
...
@pytest.mark.parametrize("config_override", [random_agent_spawn_config])
def test__randomize_scene_random_agent_spawn(ithor_env):
...
@pytest.mark.parametrize("config_override", [random_object_spawn_config])
def test__randomize_scene_random_object_spawn(ithor_env):
...
@pytest.mark.parametrize("config_override", [random_materials_config])
def test__randomize_scene_random_object_materials(ithor_env):
...
@pytest.mark.parametrize("config_override", [random_lighting_config])
def test__randomize_scene_random_lighting(ithor_env):
...
@pytest.mark.parametrize("config_override", [random_object_colors_config])
def test__randomize_scene_random_object_colors(ithor_env):
...
@pytest.mark.parametrize("config_override", [scene_randomization_config])
def test_reset_scene_randomization_materials(ithor_env):
