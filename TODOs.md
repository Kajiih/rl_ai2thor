...
- [ ] Add indirect candidate requirement for certain item prop (`water_source` necessary when `dirty=True`). Eventually make it at the same time than adding intermediate goals (toggling a `water_source`)
  ...
  - [ ] `IS_TOGGLED` -> None
  - [ ] `IS_BROKEN` -> None? Pick up?
  - [ ] `IS_FILLED_WITH_LIQUID` -> toggle a `water_source`?
  - [ ] `FILL_LIQUID` -> None (only `water` is supported)
  - [ ] `IS_DIRTY` -> toggle `water_source` if `IS_DIRTY=True`? else not possible
  - [ ] `IS_USED_UP` -> None
  - [ ] `IS_COOKED` -> toggle and/or open a compatible `cooking_source`? and pick up the `cooked` item?
  - [ ] `TEMPERATURE` -> toggle and/or open a compatible `heat_source` if `HOT` or `cold_source` if `COLD`?
  - [ ] `IS_SLICED` -> pickup `knife`
  - [ ] `IS_OPEN` -> None
  - [ ] `OPENNESS` -> None
  - [ ] `IS_PICKED_UP` -> None
  - Nothing needed for fixed properties
  - The new items generated by the intermediate goals can overlap with the original items (one candidate can work for both)
  - [ ] Add indirect candidate requirement for certain relations
    - [ ] `RECEPTACLE_OF` -> open the receptacle if it is close in the beginning
    - [ ] `CONTAINED_IN` -> None
    - [ ] `CLOSE_TO` -> None
